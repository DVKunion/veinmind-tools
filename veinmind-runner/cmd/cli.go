...
func init() {
	...
	ScanCmd.PersistentFlags().Int("threads", 5, "threads for scan action")
	ScanCmd.PersistentFlags().StringP("output", "o", "report.json", "output filepath of report")
	ScanCmd.PersistentFlags().StringP("glob", "g", "", "specifies the pattern of plugin file to find")
	// control exit
